...
function buildLocationSummaries(): LocationSummary[] {
  ...
  return cities.map((city) => {
    const match = FEATURED_PROPERTIES.find((property) => property.location.startsWith(city));

    return {
      city,
      image: match?.image ?? FEATURED_PROPERTIES[0].image,
    };
  });
}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto mb-14 max-w-2xl text-center">
          <p className="mb-3 text-xs font-medium uppercase tracking-[0.2em] text-[var(--brand-red)]">
            Where we&rsquo;re looking
          </p>
          <h2 className="text-3xl font-bold text-black sm:text-4xl lg:text-5xl">
            Target areas across the UK
          </h2>
          <p className="mt-4 text-lg text-gray-600">
            Cities we&rsquo;re researching for our first properties. Nothing is listed yet.
          </p>
        </div>
          ...
          {locations.map((location) => (
            ...
            >
              ...
              <div className="relative z-10 p-5 text-white">
                ...
                <p className="mt-1 text-sm text-white/85">Target area · no listings yet</p>
              </div>
            </Link>
          ))}
        </div>
